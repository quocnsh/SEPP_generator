...
#    create output files
    adv_text_path = r'./fool_result/{}/{}/adv.txt'.format(dataset, args.model) 
    org_text_path = r'./fool_result/{}/{}/org.txt'.format(dataset, args.model)
    adv_predict_path = r'./fool_result/{}/{}/adv_predict.txt'.format(dataset, args.model) 
    org_predict_path = r'./fool_result/{}/{}/org_predict.txt'.format(dataset, args.model) 
    adv_text_file = open(adv_text_path, "w+")
    org_text_file = open(org_text_path, "w+")    
    adv_predict_file = open(adv_predict_path, "w+")    
    org_predict_file = open(org_predict_path, "w+")    
    ...
    for index, text in enumerate(test_texts[: clean_samples_cap]):
        ...
        if np.argmax(y_test[index]) == classes_prediction[index]:
            ...
            adv_doc, adv_y, sub_rate, NE_rate, change_tuple_list, perturbed_prob = adversarial_paraphrase(input_text=text,
            ...
            if adv_y != np.argmax(y_test[index]):
                ...
                # write data to files
                adv_text_file.write(adv_doc + "\n")
                org_text_file.write(text + "\n") 
                origin_vector = text_to_vector(text, tokenizer, dataset)
                origin_prob = grad_guide.predict_prob(input_vector=origin_vector)
                org_predict = ','.join(map(str,origin_prob))                
                org_predict_file.write(org_predict + "\n")
                adv_predict = ','.join(map(str,perturbed_prob))                
                adv_predict_file.write(adv_predict + "\n")
    ...
    adv_text_file.close()
    org_text_file.close()
    adv_predict_file.close()
    org_predict_file.close()
